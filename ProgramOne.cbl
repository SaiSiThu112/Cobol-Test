           ...
           DISPLAY WS-TITLE.
                PERFORM UNTIL WS-ENDOFFILE='Y'
                 ...
                 IF WS-TEMP-BRANCH-CODE = ZERO AND
                     WS-TEMP-PRODUCT-CODE = ZERO AND
                     WS-TEMP-QUANTITY = ZERO
                     THEN
                     ADD BRANCH-CODE TO WS-TEMP-BRANCH-CODE
                     ADD PRODUCT-CODE TO WS-TEMP-PRODUCT-CODE
                ...
                *> check zero condition
                IF
                    WS-BRANCH-CODE = ZERO
                    AND WS-PRODUCT-CODE = ZERO
                    THEN
                    DISPLAY "Zero is just zero."
                END-IF

                IF WS-BRANCH-CODE = WS-TEMP-BRANCH-CODE AND
                    WS-PRODUCT-CODE = WS-TEMP-PRODUCT-CODE
                   ADD WS-QUANTITY TO WS-TOTALQUANTITY
                ELSE
                   DISPLAY "       " WS-TEMP-BRANCH-CODE
                           "           " WS-TEMP-PRODUCT-CODE
                           "       " WS-TOTALQUANTITY
